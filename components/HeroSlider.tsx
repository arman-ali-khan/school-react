...
const HeroSlider: React.FC<HeroSliderProps> = ({ items }) => {
  ...
  // Reset current if items change and index is out of bounds
  useEffect(() => {
  ...
  if (items.length === 0) {
      return <div className="w-full h-64 bg-gray-200 flex items-center justify-center text-gray-500">No slides configured</div>;
  }
      ...
      {items.map((item, index) => (
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black/80 to-transparent p-4 md:p-6">
             <p className="text-white text-lg font-medium drop-shadow-md">{item.caption}</p>
          </div>
        </div>
      ))}
            ...
            <div className="absolute bottom-2 right-4 flex gap-2">
                {items.map((_, idx) => (
                    <button 
                        ...
                        className={`w-2.5 h-2.5 rounded-full transition-colors ${idx === current ? 'bg-emerald-500' : 'bg-white/50 hover:bg-white'}`}
                    />
                ))}
            </div>
          </>
