      ...
      <div className="flex-1 overflow-y-auto notice-scroll p-0" style={{ maxHeight: '400px' }}>
        <ul className="divide-y divide-gray-100 dark:divide-gray-700">
                ...
                <li key={notice.id} className="p-3 hover:bg-emerald-50/50 dark:hover:bg-gray-700/50 transition flex items-start gap-3 group">
                    ...
                    <div className="flex-grow cursor-pointer" onClick={() => onNavigateNotice(notice.id)}>
                        ...
                        <div className="flex items-center mt-1 gap-2">
                            ...
                            <span className="text-[10px] text-gray-400 dark:text-gray-500">ID: {notice.id}2024</span>
                        </div>
                    </div>
            ...
            {displayNotices.length === 0 && (
                <li className="p-8 text-center text-gray-500 dark:text-gray-400 text-sm">No notices found in this category.</li>
            )}
        </ul>
        {filteredNotices.length > 5 && onViewAll && (
            <div className="p-2 text-center border-t border-gray-100 dark:border-gray-700">
                <button onClick={onViewAll} className="text-xs font-bold text-emerald-600 dark:text-emerald-400 hover:underline">
                    View More Notices
                </button>
            </div>
        )}
