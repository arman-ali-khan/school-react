  ...
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm h-full flex flex-col transition-colors">
      <div className="bg-emerald-50 dark:bg-gray-700/50 p-3 border-b border-emerald-100 dark:border-gray-600 flex justify-between items-center">
        <h3 className="font-bold text-emerald-800 dark:text-emerald-400 flex items-center gap-2"><div className="p-1.5 bg-emerald-600 dark:bg-emerald-700 rounded text-white"><FileText size={16} /></div>Notice Board</h3>
        {onViewAll && <button onClick={onViewAll} className="text-xs text-emerald-600 dark:text-emerald-400 hover:underline font-medium">View All</button>}
      </div>
      <div className="flex text-xs font-medium border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
        {(['all', 'general', 'student', 'college', 'exam'] as const).map((tab) => (
            <button key={tab} onClick={() => setFilter(tab)} className={`flex-1 py-2 text-center capitalize transition-colors ${filter === tab ? 'bg-white dark:bg-gray-700 text-emerald-700 dark:text-emerald-400 border-t-2 border-emerald-500 font-bold' : 'text-gray-500 dark:text-gray-400 hover:text-emerald-600 dark:hover:text-emerald-300 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>{tab}</button>
        ))}
      </div>
            ...
            {displayNotices.map((notice) => (
                <li key={notice.id} className="p-3 hover:bg-emerald-50/50 dark:hover:bg-gray-700/50 transition flex items-start gap-3 group">
                    ...
                    <div className="flex-grow cursor-pointer" onClick={() => onNavigateNotice(notice.id)}>
                        <h4 className="text-sm text-gray-800 dark:text-gray-200 font-medium group-hover:text-emerald-700 dark:group-hover:text-emerald-400 line-clamp-2">{notice.title}</h4>
                        <div className="flex items-center mt-1 gap-2">
                            ...
                            }`}>{notice.type.toUpperCase()}</span>
                        </div>
                    </div>
                    <button onClick={() => onNavigateNotice(notice.id)} className="text-gray-400 dark:text-gray-500 hover:text-emerald-600 dark:hover:text-emerald-400 p-1"><Download size={16} /></button>
                </li>
            ))}
        </ul>
      </div>
